...
export class PopupBookingServicePage implements OnInit {
  listDepartment: any;
  ...
  getListProduct() {
    ...
    this.category_bill = {};
    let total_service = 0;
    //get name of data_shop_house, logo of data_shop_house
    ...
    self.data_shop_house.group_2.forEach(object => {
      object.data.forEach(product => {
        if (product.number > 0) {
          ...
          let money_total = (parseInt(product.money.replace(/\./g, "").replace(/đ/g, "")) * product.number);
